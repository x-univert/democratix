...
export const ElectionCard = ({ election }: ElectionCardProps) => {
  ...
  const handleActivate = (e: React.MouseEvent) => {
    e.stopPropagation();
    console.log('Activer élection', election.id);
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex-1 flex flex-col gap-4">
        <h3 className="text-xl font-bold text-primary pr-32 line-clamp-2 leading-tight">
          {election.title}
        </h3>
      ...
      <div className="p-4 pt-0 border-t border-secondary bg-primary flex flex-col gap-2">
          ...
          <>
            <div className="flex gap-2 w-full">
              ...
              {canActivate && (
                ...
                >
                  {t('electionCard.actions.activateElection')}
                  {election.num_candidates < 2 && ` ${t('electionCard.actions.activateMinCandidates', { count: election.num_candidates })}`}
                </button>
              )}
            </div>
            <button
        ...
        {!isActive && (!isOrganizer || !isPending) && !shouldClose && (
          <button
